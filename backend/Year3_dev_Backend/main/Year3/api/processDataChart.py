...
def getOptionDayData(
    ...
    sensorMonitorList = 0
    if node_id == 0:
        sensorMonitorList = RawSensorMonitor.objects.filter(
            time__gt=day, room_id=room_id
        )  # time__gt = "time greater than"
    else:
        sensorMonitorList = RawSensorMonitor.objects.filter(
            time__gt=day, room_id=room_id, node_id=node_id
        )  # time__gt = "time greater than"
    list_co2 = []
    list_temp = []
    list_hum = []
...
def getOptionMonthData(
    ...
    if node_id == 0:
        sensorMonitorList = RawSensorMonitor.objects.filter(
            time__gt=time_start, time__lt=time_end, room_id=room_id
        )  # time__gt = "time greater than"
    else:
        sensorMonitorList = RawSensorMonitor.objects.filter(
            time__gt=time_start, time__lt=time_end, room_id=room_id, node_id=node_id
        )  # time__gt = "time greater than"
    ...
    table["day"] = table["datetime"].dt.day
    table["hour"] = table["datetime"].dt.hour
    table1 = table.groupby(["day"], as_index=False).mean(
        numeric_only=True
    ...
    for row in table1.itertuples(index=False):
        listTime.append(
            f"{str(int(float(str(row.day))))}/{str(int(float(str(row.month))))}/{str(int(float(str(row.year))))}"
        )
    print(listTime)
...
def getOptionYearData(
    ...
    table["day"] = table["datetime"].dt.day
    table["hour"] = table["datetime"].dt.hour
    table1 = table.groupby(["month"], as_index=False).mean(
        numeric_only=True
    )  # group all data by "day"
    print(table1)
    ...
    for row in table1.itertuples(index=False):
        listTime.append(
            f"{str(int(float(str(row.month))))}/{str(int(float(str(row.year))))}"
        )
    print(listTime)
