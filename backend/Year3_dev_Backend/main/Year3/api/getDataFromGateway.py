...
def insert_to_DB(
    ...
    if topic == backend_topic_dictionary["get_sensor_data"]:
        ...
        if data["operator"] == "data_response":
            ...
            for i in parameter_key_list:
                if i not in data["info"]:
                    ...
                    record = record + (
                        data["info"][i],
                    )  #!< create a tupble of one element "data["info"][i]" and concatenate it to record
            print(record)
            ...
            print("Successfully insert SENSORRR to PostgreSQL")
            print(
                f"Date_time: {datetime.fromtimestamp(data['info']['time'] - 7*60*60)}"
            )
            cursor.close()
            conn.close()
        else:
        ...
        if (
            data["status"] == 0
        ):  #!< status = 0 means that gateway will send all nodes available to server
            ...
            all_rooms_in_database_list = (
                cursor.fetchall()
            )  #!< a list that contains all room_id that available in database
            current_room_id_in_database_list = [
                i["room_id"] for i in all_rooms_in_database_list
            ]
            ...
            current_node_id_list = [
                i["node_id"] for i in all_nodes_in_room_list
            ]  #!< get all current ids of nodes
            new_node_data: list = (
                []
            )  #!< this contains all the new nodes whichs will be inserted into database
            for node in data["info"][
                "node_list"
            ]:  #!< get all the new nodes which is not present in database
                if node["node_id"] not in current_node_id_list:
                    new_node_data.append(node)
                else:
                    continue
            if (
                new_node_data.__len__() > 0
            ):  #!< if there are new nodes needed to be inserted into database
            ...
            data["status"] == 1
        ):  #!< status = 1 means that gateway will add one new node to server
            from psycopg2.extras import DictCursor
            ...
            data["status"] == 2
        ):  #!< status = 2 means that gateway will send list of sensors need to be deleted from database
            from psycopg2.extras import DictCursor
            ...
            node_to_be_deleted_list: list = (
                []
            )  #!< this contains all the nodes whichs gateway wants server to delete
            for node in data["info"][
                "node_list"
            ]:  #!< get all nodes which gateway wants server to delete
                if (
                    node["node_id"] in current_node_id_list
                ):  #!< if node to be deleted presents in database, add it to list
...
def run(topic):
    ...
    client.connect(
        broker, int(1883), 60
    )  # <! this will automatically subscribe to the topic
        ...
        while 1:
            try:
                ...
                if temp != None:
                    ...
                    client.publish(
                        backend_topic_dictionary["room_sync_gateway_backend"],
                        json.dumps(msg_response),
                    )
            except:
        ...
        while 1:
            try:
                ...
                if temp != None:
                    ...
                    insert_to_DB(
                        topic, msg, "smartfarm", "year3", "year3", "my-postgres", "5432"
                    )
            except:
                print(
                    "!!!!!!Error in getDataFromGateway.py while insert data to database"
                )
                time.sleep(10)
                ...
                for i in dict_key:
                    if i == "time":
                        ...
                        if len(all_data_in_aqiref_orderby_time_desc) == 0:
                            ...
                            latest_data_in_database = (
                                all_data_in_aqiref_orderby_time_desc[0]
                            )  #!< tuple
                            print(latest_data_in_database)
                            if int(time_of_data) == int(
                                latest_data_in_database[0]
                            ):  #!< time is in index 11 of tuple lastest_data_in_database
...
if __name__ == "__main__":
    ...
    process_list.append(
        multiprocessing.Process(
            target=run, args=(backend_topic_dictionary["room_sync_gateway_backend"],)
        )
    )  #!< must have ',' in the finishing of set args
    process_list.append(
        multiprocessing.Process(
            target=run, args=(backend_topic_dictionary["get_sensor_data"],)
        )
    )  #!< must have ',' in the finishing of set args
    process_list.append(
        multiprocessing.Process(
            target=run, args=(backend_topic_dictionary["get_actuator_data"],)
        )
    )  #!< must have ',' in the finishing of set args
